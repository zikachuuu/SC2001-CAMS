...
private static void writeSuggestionToFile(Suggestion suggestion) {
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(SUGGESTIONS_FILE_PATH, true))) {
        writer.write(suggestion.getStudentId() + "," + suggestion.getCampName() + "," +suggestion.
                getSuggestionsText()+ "," + suggestion.isApproved());                
        writer.newLine();
    } catch (IOException e) {
        e.printStackTrace();
    }
